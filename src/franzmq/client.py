...
from franzmq.payload import Payload, ServiceDetails
...
class Client(PahoClient):
    ...
    def autocreate_and_connect(cls, client_id: str, on_connect: Callable = None, on_disconnect: Callable = None):
        ...
        ca_cert = config("CA_CERT_FILE", default="/etc/certs/prekit-ca.crt", cast=path_or_none)
        tls_cert = config("TLS_CERT_FILE", default="/etc/certs/prekit-tls.pem", cast=path_or_none)
        tls_key = config("TLS_KEY_FILE", default="/etc/certs/prekit-tls.key", cast=path_or_none)
